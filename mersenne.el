...
(defclass Random ()
  ((w :initform 32 :type number :documentation "Word size")
   ...
   (m :initform 397 :type number :documentation "Middle word, an offset used in the recurrence relation")
   (r :initform 31 :type number :documentation "Separation point of one word, or the number of bits of the lower bitmask")
   (a :initform #x9908B0DF :type number :documentation "Coefficients of the rational normal form twist matrix")
   (u :initform 11 :type number :documentation "Additional Mersenne Twister tempering bit shift/mask")
   ...
   (b :initform #x9D2C5680 :type number :documentation "TGFSR(R) tempering bitmask")
   ...
   (f :initform 1812433253 :type number :documentation "Initialization multiplier")
   (MT :initform (make-vector 624 0) :type vector :documentation "The array for the state vector")
   (index :initform 625 :type number :documentation "Current index in the state vector")
   ...
   (c_seed :initarg :c_seed :type number :documentation "Current seed for the generator"))
  "A class representing a Mersenne Twister pseudorandom number generator.")

(cl-defmethod initialize-instance :after ((obj Random) &rest args)
  ...
  ;; Initialize the seed
  (let ((seed (plist-get args :c_seed)))
    (when seed
      (oset obj c_seed seed)
      (seed obj seed)))
  ;; Call next method if necessary
  (if (cl-next-method-p)
      (cl-call-next-method)))
...
(cl-defmethod seed ((obj Random) num)
  ...
  (cl-loop for i from 1 below (oref obj n) do
           (let* ((prev (aref (oref obj MT) (1- i)))
                  (temp (+ (* (oref obj f) (logxor prev (lsh prev (- (oref obj w) 2))))
                           i)))
...
(cl-defmethod twist ((obj Random))
  ...
  (cl-loop for i from 0 below (oref obj n) do
           (let* ((x (+ (logand (aref (oref obj MT) i) (oref obj upper_mask))
                        (logand (aref (oref obj MT) (mod (1+ i) (oref obj n))) (oref obj lower_mask))))
                  (xA (lsh x -1)))
             ;; Apply the twist transformation
             (when (/= (mod x 2) 0)
               (setq xA (logxor xA (oref obj a))))
             (aset (oref obj MT) i (logxor (aref (oref obj MT) (mod (+ i (oref obj m)) (oref obj n))) xA)))
           (oset obj index 0)))
...
(cl-defmethod mt-random ((obj Random))
  ...
  (/ (extract_number obj) (expt 2.0 32)))

(cl-defmethod randint ((obj Random) a b)
  "Return a random integer in the range [a, b)."
  (let ((range (- b a))
        (random-value (mt_random obj)))
